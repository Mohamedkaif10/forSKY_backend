    ...
    const formattedJobDetails = result.rows.map((row) => {
      if (row.last_date) {
        ...
        row.last_date = `${
          ...
          date.toLocaleDateString("en-US", options).split(" ")[2]
        }`;
      } else {
...
const getIdeasByStream = async (stream) => {
  try {
    const result = await db.query("SELECT * FROM ideas WHERE stream = $1", [
      stream,
    ]);
    return result.rows;
  } catch (error) {
...
const getBookmarks = async (userId) => {
  try {
    const bookmarks = await db.query(
      "SELECT jobdetails.* FROM jobdetails JOIN bookmarks ON jobdetails.job_id = bookmarks.job_id WHERE bookmarks.user_id = $1",
      [userId]
    );
