...
router.put('/update-role-name', dynamicJwtCheck, async(req,res) =>{
    try {
        ...
        if(email !== req.body.email){
            return res.status(400).json({message: "You are not the owner of this email"})
        }
    
        const checkExit = await client.query("SELECT EXISTS(SELECT 1 FROM users WHERE email = $1)", [email]);
        if(!checkExit.rows[0].exists){
            return res.status(400).json({message: "User doesn't exit"});
        }
    
        const result = await client.query(`UPDATE users SET role = $1 WHERE email = $2`, [req.body.role, email]);
